...
class ManagerReportCalculator implements CalculationServiceInterface
{
    ...
    public function calculate($data): array
    {
        ...
        return [
            ...
            //pourcentages par rapport au cntrl effectifs
            'executionsNotAppliedPctPerIneff' => round((($first['executions_not_applied'] ?? 0) / max(1, $first['controls_ineffectifs'])) * 100),
            'executionsPartiallyAppliedPctPerIneff' => round((($first['executions_partially_applied'] ?? 0) / max(1, $first['controls_ineffectifs'])) * 100),
            'executionsNotTestedPctPerIneff' => round((($first['executions_not_tested'] ?? 0) / max(1, $first['controls_ineffectifs'])) * 100),
            'executionsNotApplicablePctPerIneff' => round((($first['executions_not_applicable'] ?? 0) / max(1, $first['controls_ineffectifs'])) * 100),
